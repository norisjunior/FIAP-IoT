...
#define MOTOR_INA     4   // ponte H do mini motor 3V: gira enquanto a coleta roda
#define MOTOR_INB    19
  ...
  if (ultimoBotaoColeta == HIGH && leituraColeta == LOW) {
    if (millis() - ultimoDebounceColeta > debounceMs) {
      ...
      if (coletando) {
        ...
        digitalWrite(MOTOR_INA, HIGH);   // liga o motor enquanto coleta
        digitalWrite(MOTOR_INB, LOW);
        Serial.printf("Coleta INICIADA (classe: %s)\r\n", SEQUENCIA[indiceClasse]);
      } else {
        digitalWrite(LED_ONBOARD, LOW);
        digitalWrite(MOTOR_INA, LOW);    // desliga o motor
        digitalWrite(MOTOR_INB, LOW);
