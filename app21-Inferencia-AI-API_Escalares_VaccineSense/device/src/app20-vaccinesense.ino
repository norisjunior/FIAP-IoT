...
// Chega o resultado do modelo, vindo do n8n.
// Nao ha nenhuma regra aqui: o dispositivo so obedece.
void receberComando(char* topico, byte* conteudo, unsigned int tamanho) {
  String comando(conteudo, tamanho);
  comando.trim();
  comando.toUpperCase();

  Serial.println(">>> Comando da nuvem: " + comando);
...
void setup() {
  ...
  Serial.println("Vaccine Sense pronto.");
  Serial.println("Publica em: " + String(TOPICO));
  Serial.println("Escuta em:  " + String(TOPICO_CMD));
