...
void setup() {
  ...
  // Fase 3: identificar e testar com FastIMU
  switch (whoami) {
    case 0x68: {
      ...
      if (imu.init(calib, 0x68) == 0) {
        Serial.println("Status: OK via FastIMU MPU6050");
        imu.update();
        AccelData accel;
        imu.getAccel(&accel);
        ...
        Serial.println("Status: FALHA no init()");
      }
    ...
    case 0x70: {
      ...
      calData calib = {0};
      if (imu.init(calib, 0x68) == 0) {
        Serial.println("Status: OK via FastIMU MPU6500");
        imu.update();
        AccelData accel;
        imu.getAccel(&accel);
        Serial.printf("Teste: ax=%.2fg ay=%.2fg az=%.2fg\n",
    ...
    case 0x72: {
      Serial.println("Chip: IMU generica compativel com MPU6500/MPU9250");
      Wire.setClock(400000);
      IMU_Generic imu(Wire);
      calData calib = {0};
      if (imu.init(calib, 0x68) == 0) {
        Serial.println("Status: OK via FastIMU IMU_Generic");
        imu.update();
        AccelData accel;
        imu.getAccel(&accel);
        Serial.printf("Teste: ax=%.2fg ay=%.2fg az=%.2fg\n",
    ...
    case 0x71:
      ...
      Serial.println("Use a classe MPU9250 do FastIMU.");
      break;
    case 0x75:
      Serial.println("Chip: MPU6515");
      Serial.println("Use a classe MPU6515 do FastIMU.");
      break;
    case 0xEA:
      Serial.println("Chip: ICM-20948");
      Serial.println("Use a classe ICM20948 do FastIMU.");
      break;
    default:
