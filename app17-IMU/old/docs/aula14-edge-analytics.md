# Aula 14 — Da série temporal ao Edge Analytics

## Objetivo do documento

Este documento resume a base conceitual que deve orientar os códigos da Aula 14.

O código gerado para esta aula deve refletir as decisões didáticas abaixo:

- vibração é sinal temporal;
- dados raw são a fonte da verdade;
- dados raw permitem visualização e reprocessamento;
- features reduzem volume de dados;
- Edge Analytics processa perto do sensor;
- janela temporal é decisão de engenharia;
- eventos curtos podem sofrer efeito de borda;
- janela com sobreposição reduz risco de perda de eventos;
- labels vêm do protocolo experimental;
- timestamp permite organizar a coleta no tempo;
- datasets raw e datasets de features têm granularidades diferentes;
- separação treino/teste deve evitar vazamento de informação.
