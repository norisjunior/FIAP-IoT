# service_app.py - a Random Forest da Smart Delivery Bag vira servico.
#
# Recebe as SEIS features escalares medidas a bordo no intervalo de 1 s pelo
# e devolve a classe. E o mesmo modelo que o app da floresta embarcada roda: aqui
...
app = FastAPI(title="Smart Delivery Bag: revisar a entrega?")
...
if not os.path.exists(MODELO_ARQUIVO):
    raise SystemExit(
        ...
        "Para gerar o seu:\n"
        "  1. Colete duas rodadas completas com o app de coleta.\n"
        "  2. Rode coleta_e_rotulagem.ipynb e baixe o CSV.\n"
        "  3. Rode treinamento_smartbag_rf.ipynb ate o fim.\n"
...
# app da floresta embarcada. O DataFrame abaixo casa por NOME, entao trocar a ordem aqui nao
# quebraria a API -- mas quebraria a comparacao com o app da floresta embarcada, que casa por
...
# O treino mapeou os nomes para 0 e 1 porque o micromlgen, no app da floresta embarcada, so
...
def predict(amostra: AmostraBag):
    ...
    # INTEIRO 0 ou 1 -- diferente do app do motor, onde o treino usava rotulo em texto
